...
class Insomnia : public IOService, InsomniaSettings::Watcher {
    OSDeclareDefaultStructors(Insomnia);

public:
    virtual bool init(OSDictionary * = 0) APPLE_KEXT_OVERRIDE;
...
protected:
    ...
    bool property_changed(const struct InsomniaSettings::PropertyChangeEvent * event) APPLE_KEXT_OVERRIDE;

private:
    InsomniaSettings settings;
    ...
    virtual IOReturn setPowerState(unsigned long whichState, IOService * whatDevice) APPLE_KEXT_OVERRIDE;
